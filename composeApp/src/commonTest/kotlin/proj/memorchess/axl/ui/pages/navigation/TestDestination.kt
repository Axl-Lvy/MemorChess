...
/**
 * Serialization coverage for [Route]'s scoped variants, which Compose Navigation relies on to carry
 * a route's arguments across a process death. Round trips go through each concrete type's own
 * generated serializer, exactly the way `composable<Route.TrainingRoute>`/`toRoute` in `Router.kt`
 * use it, not the polymorphic `Route` serializer (which this sealed interface, unlike a sealed
 * class, needs an explicit `SerializersModule` to support and neither Compose Navigation nor this
 * app ever registers one).
 */

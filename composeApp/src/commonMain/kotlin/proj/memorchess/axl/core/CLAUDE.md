...
- **`graph/`** — the opening tree, keyed by `PositionKey`. **`TreeStore` is the single chokepoint for local mutation** — never mutate the graph anywhere else; remote writes land through `SyncApplier`. `NodeCache` owns the in-memory `OpeningTree` and its lock, and is the only thing that mutates it: `Prefetcher` warms neighbours through it, `TrainableProjection` maintains the per-repertoire trainable rows, and `RepertoireTagStore` owns the registry and edge tags. `DeleteMode.SOFT` is the default (a tombstone may need to reach another device); `HARD` is for local-only cleanup. `eraseAll` wipes everything unconditionally, bypassing `DeleteMode`.
...
- **`data/`** — the persistence seam. `DatabaseQueryManager` has Room (`nonJsMain`), IndexedDB (`wasmJsMain`), and in-memory (tests) implementations; only `TreeStore`, `SyncApplier`, `RepertoireTagStore`, `TrainableProjection` and those implementations may touch its node/move surface. Every write that changes a node or move queues its own outbox entry in the same transaction as the row, so a crash between the two can never drop one relative to the other. `PositionKey` lives in `:shared`.
